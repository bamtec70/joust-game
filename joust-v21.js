/**
 * JOUST — Williams Electronics 1982 (Build V21)
 * V21: arcade board/sprites/gameplay/audio polish. Cliff X = V19 (byte×2×3). Egg physics = V15.
...
(() => {
  ...
  function sfx(name) {
    unlockAudio();
    if (muted || !AC) return;
    // V21: Web Audio synth is primary (always audible). Optional cabinet wav layers only.
  ...
  const BASE_PLATFORMS = [
    ...
    { x: 0, y: 633, w: 162, h: 10, kind: "bridge", cliff: "BRIDGE_L" },
    { x: 162, y: 633, w: 558, h: 54, kind: "ground", cliff: "CLIF5" },
    { x: 720, y: 633, w: 156, h: 10, kind: "bridge", cliff: "BRIDGE_R" },
  ];
  ...
  const GRAV = 505; // V21: slightly heavier toward arcade drop
  const FLAP_VY = -128; // V21: stronger single flap (arcade punch)
  const MAX_FALL = 310;
  const MAX_RISE = -290;
  const ACCEL_X = 420;
  const MAX_VX = 198;
  const GROUND_FRIC = 0.76; // V21: more platform skid
  ...
  function layoutForWave(n) {
    ...
    const drop = new Set();
    if (miss.includes("1")) drop.add("CLIF1L");
    if (miss.includes("2")) drop.add("CLIF1R");
    if (miss.includes("3")) drop.add("CLIF2");
  ...
  function goTitle() {
    ...
    ptero = null;
    hands = [];
    showOV("JOUST", "INSERT COIN", "BUILD V21 â€” INSERT COIN â€” PRESS SPACE / FLAP");
    hud();
  }
    ...
    if (onLava(r.x, r.y + 10)) {
      if (isPlayer) {
        if (wave >= 4) {
          ...
          r.grab = 520; // V21: longer troll struggle
          r.vy = 40;
        } else {
          sfx("lava");
          killPlayer();
  ...
  function pteroMouthOpen(pt) {
    ...
    return Math.sin(pt.mouthPhase) > 0.05; // V21: mouth open window slightly wider
  }
  ...
  function update(dt) {
    ...
    if (player && player.grab) {
      ...
      player.grab -= dt;
      if (flapPulse) {
        player.grab -= 165; // V21: flap escapes troll faster
  ...
  function rockShelf(p) {
    ...
    if (bridge) {
      // Thin bridge rails flush with CLIF5 top — slightly richer than V19
      const bh = Math.min(h, 10);
      const g = ctx.createLinearGradient(0, y, 0, y + bh);
      g.addColorStop(0, "#e09858");
      g.addColorStop(0.35, "#a05828");
  ...
  function drawEgg(egg) {
    // V21: rounder glowing arcade egg (visual only; physics = V15)
  ...
  function drawOnePtero(pt) {
    ...
    // V21: rounder / slightly larger arcade ptero
    const s = 1.0;
  ...
  // V21: resume AudioContext on any first user gesture (silence-bug guard)
