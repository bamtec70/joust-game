/**
 * JOUST â€” Williams Electronics 1982 (Build V19)
 * V19: exact 3Ã— arcade (876Ã—720) cliff board from playfield ref. Sprites = V16, egg physics = V15.
...
(() => {
  ...
  function sfx(name) {
    unlockAudio();
    if (muted || !AC) return;
    // Keep start sting clear â€” don't let wave/extra stomp it
  ...
  function goTitle() {
    ...
    ptero = null;
    hands = [];
    showOV("JOUST", "INSERT COIN", "BUILD V19 â€” INSERT COIN â€” PRESS SPACE / FLAP");
    hud();
  }
  ...
  function rockShelf(p) {
    ...
    // Float shelves: THIN â€” bright top edge + short jagged underside (8â€“14px)
    const shelf = Math.min(Math.max(h, 12), 16);
    const under = 10 + (Math.sin(x * 0.03) * 2 | 0); // 8â€“12-ish jagged depth feel
    ctx.beginPath();
    ...
    // Jagged rocky underside (short â€” not fat bricks)
