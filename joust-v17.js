/**
 * JOUST — Williams Electronics 1982 (Build V17)
 * Arcade Williams cliff board layout (joustr). Sprites = V16, egg physics = V15.
...
(() => {
  ...
  // Platforms — Williams arcade cliffs scaled 292×240 → 960×540
  const SX = 960 / 292;
  const SY = 540 / 240;
  const ax = (x) => Math.round(x * SX);
  const ay = (y) => Math.round(y * SY);
  const aw = (w) => Math.round(w * SX);
  const ah = (h) => Math.max(16, Math.round(h * SY));

  const BASE_PLATFORMS = [
    // Top ledges
    { x: ax(1), y: ay(69), w: aw(34), h: ah(14), kind: "float", cliff: "CLIF1L" },
    { x: ax(126), y: ay(69), w: aw(48), h: ah(14), kind: "float", cliff: "CLIF1R" },
    // Long upper-mid shelf
    { x: ax(43), y: ay(81), w: aw(88), h: ah(16), kind: "float", cliff: "CLIF2" },
    // Mid row
    { x: ax(101), y: ay(129), w: aw(58), h: ah(16), kind: "float", cliff: "CLIF3U" },
    { x: ax(1), y: ay(138), w: aw(64), h: ah(16), kind: "float", cliff: "CLIF3L" },
    { x: ax(127), y: ay(138), w: aw(48), h: ah(16), kind: "float", cliff: "CLIF3R" },
    // Lower-center float
    { x: ax(53), y: ay(163), w: aw(64), h: ah(16), kind: "float", cliff: "CLIF4" },
    // Bottom: thin bridges left/right of CLIF5 over lava (collapse after wave ~3)
    { x: ax(0), y: ay(211), w: ax(27) - ax(0), h: 20, kind: "bridge", cliff: "BRIDGE_L" },
    // Tall center HUD rock island rising out of lava
    { x: ax(27), y: ay(211), w: aw(186), h: 80, kind: "ground", cliff: "CLIF5" },
    { x: ax(27) + aw(186), y: ay(211), w: Math.max(80, ax(292) - (ax(27) + aw(186))), h: 20, kind: "bridge", cliff: "BRIDGE_R" },
  ];
  let PLATFORMS = BASE_PLATFORMS.map((p) => Object.assign({}, p));

  // Continuous lava band across full bottom under/around CLIF5
  const LAVA_LINE = ay(211) + 8;
  ...
  function onLava(x, y) {
    // Arcade: continuous lava under bottom; safe on CLIF5 / bridges / any ledge
    if (y < LAVA_LINE - 8) return false;
    for (const p of PLATFORMS) {
      if (x >= p.x - 4 && x <= p.x + p.w + 4 && y <= p.y + 18 && y >= p.y - 24) {
        return false;
      }
    }
  ...
  function spawnPlayer(safe) {
    // Prefer CLIF5 island top; fall back to CLIF4 then any ledge
  ...
  function layoutForWave(n) {
    ...
    const restored = n % 5 === 0; // egg waves restore full layout (Williams)
    if (!restored) {
      // After wave ~3+: bridges collapse — lava opens under sides of CLIF5
      if (n >= 3) {
        list = list.filter((p) => p.kind !== "bridge");
      }
      // WBCL2-ish: remove CLIF2
      if (n >= 6) {
        list = list.filter((p) => p.cliff !== "CLIF2");
      }
      // WBCL1-ish: remove CLIF1L + CLIF1R
      if (n >= 7) {
        list = list.filter((p) => p.cliff !== "CLIF1L" && p.cliff !== "CLIF1R");
      }
      // WBCL4 later: remove CLIF4
      if (n >= 8) {
  ...
  function beginWave(n) {
    ...
    // Enemy spawns on remaining platforms (arcade materialize pads)
    const spawnSpots = PLATFORMS.filter((p) => p.cliff !== "BRIDGE_L" && p.cliff !== "BRIDGE_R").map((p) => ({
      x: p.x + p.w * 0.5,
      y: p.y - 28,
    }));
    if (!spawnSpots.length) {
      spawnSpots.push({ x: VW * 0.5, y: VH * 0.4 });
    }
  ...
  function goTitle() {
    ...
    ptero = null;
    hands = [];
    showOV("JOUST", "INSERT COIN", "BUILD V17 — INSERT COIN — PRESS SPACE / FLAP");
    hud();
  }
  ...
  function rockShelf(p) {
    ...
    const tall = ground && h >= 50;
    ctx.beginPath();
    if (tall) {
      // Big jagged bottom island (score-pad feel) rising from lava
      ctx.moveTo(x, Math.min(VH, y + h));
      ctx.lineTo(x - 4, y + h * 0.55);
      ctx.lineTo(x, y + 8);
      const steps = Math.max(8, (w / 22) | 0);
      for (let i = 0; i <= steps; i++) {
        const t = i / steps;
        const px = x + t * w;
        const jag = Math.sin(i * 1.9 + x * 0.02) * 4 + Math.sin(i * 0.7) * 2;
        const edge = i === 0 || i === steps ? 3 : 0;
        ctx.lineTo(px, y + jag + edge);
      }
      ctx.lineTo(x + w, y + 8);
      ctx.lineTo(x + w + 4, y + h * 0.55);
      ctx.lineTo(x + w, Math.min(VH, y + h));
      ctx.closePath();
    } else {
      ctx.moveTo(x, y + h);
      ctx.lineTo(x, y + (bridge ? 3 : 6));
      const steps = Math.max(4, (w / 28) | 0);
      for (let i = 0; i <= steps; i++) {
        const t = i / steps;
        const px = x + t * w;
        const jag = Math.sin(i * 1.7 + x * 0.01) * (bridge ? 1.5 : 2.5);
    ...
    ctx.lineWidth = tall ? 2 : 1.5;
    ctx.beginPath();
    const steps2 = Math.max(4, (w / (tall ? 22 : 28)) | 0);
    for (let i = 0; i <= steps2; i++) {
      const t = i / steps2;
      const px = x + t * w;
      const jag = Math.sin(i * (tall ? 1.9 : 1.7) + x * 0.01) * (tall ? 4 : bridge ? 1.5 : 2.5);
    ...
    if (!bridge) {
      ...
      for (let i = 10; i < w - 10; i += 18) {
        oval(x + i, y + h * 0.35, 2.2, 1.3, C.rockMid);
      }
    }
    ...
    if (p.cliff === "CLIF2" || p.cliff === "CLIF3U") {
      ...
      ctx.fillRect(px1, y - 2, padW, 5);
      ctx.fillRect(px2, y - 2, padW, 5);
      ctx.fillStyle = "rgba(210,210,220,0.35)";
      ctx.fillRect(px1 + 2, y - 1, padW - 4, 2);
      ctx.fillRect(px2 + 2, y - 1, padW - 4, 2);
    }
  }
  ...
  function drawLava() {
    ...
    const L = LAVA[0];
    // Full-width glowing band behind CLIF5
    fillRect(0, LAVA_LINE - 6, VW, VH - (LAVA_LINE - 6), "#1a0500");
    const glow = ctx.createLinearGradient(0, LAVA_LINE - 40, 0, VH);
    glow.addColorStop(0, "rgba(255,80,0,0)");
    glow.addColorStop(0.35, "rgba(255,100,0,0.18)");
    glow.addColorStop(1, "rgba(255,40,0,0.45)");
    ctx.fillStyle = glow;
    ctx.fillRect(0, LAVA_LINE - 40, VW, VH - (LAVA_LINE - 40));
    const lg = ctx.createLinearGradient(0, LAVA_LINE, 0, VH);
    lg.addColorStop(0, C.lavaCore);
    lg.addColorStop(0.18, C.lavaHot);
    ...
    for (let i = 0; i <= segs; i++) {
      ...
      ctx.lineTo(
        t * VW,
        LAVA_LINE + Math.sin(t * Math.PI * 5 + lavaAnim / 100) * 4 * pulse
      );
    }
    ctx.lineTo(VW, VH);
    ctx.lineTo(0, VH);
    ...
    for (let i = 0; i < 18; i++) {
      ...
      const by = LAVA_LINE + 6 + ((i * 7 + lavaAnim / 45) % 22);
