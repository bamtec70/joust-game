/**
 * JOUST — Williams Electronics 1982 (Build V20)
 * V20: arcade board/sprites/gameplay/audio polish. Cliff X = V19 (byte×2×3). Egg physics = V15.
...
(() => {
  ...
  function sfx(name) {
    unlockAudio();
    if (muted || !AC) return;
    // V20: Web Audio synth is primary (always audible). Optional cabinet wav layers only.
  ...
  const GRAV = 505; // V20: slightly heavier toward arcade drop
  const FLAP_VY = -128; // V20: stronger single flap (arcade punch)
  const MAX_FALL = 310;
  const MAX_RISE = -290;
  const ACCEL_X = 420;
  const MAX_VX = 198;
  const GROUND_FRIC = 0.76; // V20: more platform skid
  ...
  function enemyTypeForWave(w, i) {
    // Bounder â†’ Hunter â†’ Shadow Lord mix ramps
  ...
  function layoutForWave(n) {
    ...
    if (!restored) {
      // nâ‰¥3: drop bridges â€” lava opens under sides of CLIF5
      if (n >= 3) {
  ...
  /** Original-style: wave 8, then every 5th (13, 18, 23â€¦) is a Pterodactyl wave. */
  ...
  function beginWave(n) {
    ...
    survivedWave = true;
    layoutForWave(n);
    // Hurry-up: if wave drags on, a pterodactyl hunts the player (original ~20â€“45s).
  ...
  function goTitle() {
    ...
    ptero = null;
    hands = [];
    showOV("JOUST", "INSERT COIN", "BUILD V20 â€” INSERT COIN â€” PRESS SPACE / FLAP");
    hud();
  }
    ...
    if (onLava(r.x, r.y + 10)) {
      if (isPlayer) {
        if (wave >= 4) {
          ...
          r.grab = 520; // V20: longer troll struggle
          r.vy = 40;
        } else {
          sfx("lava");
          killPlayer();
  ...
  function pteroMouthOpen(pt) {
    ...
    return Math.sin(pt.mouthPhase) > 0.05; // V20: mouth open window slightly wider
  }

  function pteroMouthPos(pt) {
    // Facing: positive vx = flying right, mouth on right side
    const face = pt.vx >= 0 ? 1 : -1;
    const s = 1.0; // V20 ptero visual // match drawOnePtero scale
    return {
      x: pt.x + face * 30 * s,
      y: pt.y + 0.5 * s,
      face,
  ...
  function updatePteros(dt) {
    ...
    for (const pt of pteros) {
      ...
      if (player && player.alive && invuln <= 0) {
        ...
        const spearMouth = open && mouthDx < 17 && mouthDy < 12; // V20 mouth hit
  ...
  function update(dt) {
    ...
    if (player && player.grab) {
      ...
      player.grab -= dt;
      if (flapPulse) {
        player.grab -= 165; // V20: flap escapes troll faster
  ...
  function rockShelf(p) {
    ...
    // Float shelves: THICKER rounded rocky tops (V20 arcade feel)
  ...
  function drawRider(r, color, dark, wingCol) {
    ...
    const s = 0.78; // V20 visual scale-up
  ...
  function drawEgg(egg) {
    // V20: rounder glowing arcade egg (visual only; physics = V15)
  ...
  function drawOnePtero(pt) {
    ...
    // V20: rounder / slightly larger arcade ptero
    const s = 1.0;
  ...
  // V20: resume AudioContext on any first user gesture (silence-bug guard)
