...
(() => {
  ...
  // Pixel plotter (1 unit ≈ one arcade-ish pixel at our scale)
  ...
  /**
   * Draw mount + knight as readable pixel silhouettes (not egg blobs).
   * Facing +X locally; mirrored by scale(face).
   */
  function drawBirdLegs(r, walking) {
    const leg = C.yellow;
    if (!walking) {
      // Flight: legs tucked back under belly
      px(-5, 5, 2, 5, leg);
      px(-6, 9, 3, 2, leg);
      px(2, 5, 2, 5, leg);
      px(1, 9, 3, 2, leg);
    ...
    // 4-frame walk from walkPhase
    const frame = ((r.walkPhase || 0) * 4) | 0;
    const f = frame % 4;
    // hip at y=4 under body
    // frame: which leg forward
    const cycles = [
      // [backLeg: hipX, footX, footY], [frontLeg...]
      [
        [-4, -7, 12],
        [2, 6, 11],
      ],
    ...
    const [L, R] = cycles[f];
    function legDraw(hipX, footX, footY) {
      // upper
      px(hipX, 4, 2, 4, leg);
      // lower
      const midX = ((hipX + footX) / 2) | 0;
      px(midX, 7, 2, 3, leg);
      // foot / toes
      px(footX - 1, footY, 4, 2, leg);
      px(footX + 2, footY, 2, 1, leg);
    }
    legDraw(L[0], L[1], L[2]);
    legDraw(R[0], R[1], R[2]);
  }
  ...
  function drawRider(r, color, dark, wingCol) {
    ...
    const s = 1.15; // readable on full-screen landscape

    ctx.save();
    ctx.translate(r.x | 0, r.y | 0);
    ctx.scale(f * s, s);

    // ═══════ BIRD / BUZZARD (body points right) ═══════
    // Tail
    px(-12, 0, 4, 3, dark);
    px(-14, 1, 3, 2, dark);
    // Main body (horizontal oval built from rects)
    px(-8, -2, 14, 8, color);
    px(-6, -3, 11, 2, wingCol || color);
    px(-9, 0, 5, 5, dark);
    px(-4, 4, 10, 3, color);
    // Breast
    px(2, 0, 5, 5, wingCol || color);

    // Long neck (distinctive Joust silhouette — not a blob)
    px(6, -4, 3, 5, color);
    px(7, -8, 3, 5, color);
    px(8, -11, 4, 4, color);
    // Head
    ...
    px(11, -15, 4, 2, color);
    // Beak
    px(15, -13, 4, 2, C.yellow);
    px(18, -12, 2, 1, C.yellow);
    // Eye
    px(12, -13, 2, 2, "#101010");
    px(13, -13, 1, 1, "#ffffff");
    ...
    if (flap) {
      // raised
      px(-6, -8, 8, 3, dark);
      px(-4, -11, 9, 4, wingCol || color);
      px(-2, -13, 6, 2, dark);
    } else if (r.onGround) {
      // folded against side
      px(-5, 1, 7, 4, dark);
      px(-4, 2, 5, 2, wingCol || color);
    } else {
      // gliding slightly down
      px(-7, 3, 9, 3, dark);
      px(-5, 5, 7, 2, wingCol || color);
    }

    // Legs (under body)
    drawBirdLegs(r, walking);

    // ═══════ KNIGHT (clear human silhouette — NOT an egg) ═══════
    // Sits upright on the saddle (mid-back of bird)
    // Legs dangling / straddling
    px(-2, -5, 2, 5, C.knight); // left leg
    px(2, -5, 2, 5, C.knight); // right leg
    px(-3, -1, 2, 2, "#c0c0c0"); // boot
    px(3, -1, 2, 2, "#c0c0c0");

    // Torso (rectangular, armored)
    px(-3, -11, 7, 7, C.knight);
    px(-2, -10, 5, 5, "#d8d8d8");
    // Belt
    px(-3, -5, 7, 1, "#808080");

    // Shoulders / arms
    px(-5, -10, 2, 4, C.knight); // back arm
    px(4, -11, 3, 3, C.knight); // lance arm forward
    px(5, -10, 4, 2, C.knight);

    // Head + helmet (distinct from body)
    px(-2, -16, 5, 5, C.knight);
    px(-1, -15, 3, 3, "#b0b0b0"); // face opening
    // Visor slit
    px(-1, -14, 3, 1, "#202020");
    // Helmet crest / plume (enemy color or yellow for player)
    let plume = C.yellow;
    if (color === C.hunter) plume = C.hunter;
    else if (color === C.shadow) plume = C.shadow;
    else if (color === C.bounder) plume = "#a0e060";
    px(0, -19, 2, 3, plume);
    px(-1, -18, 4, 1, plume);

    // Lance (long horizontal spear — hallmark of Joust)
    px(6, -12, 14, 2, C.lance);
    px(19, -13, 3, 4, C.yellow); // tip
    px(5, -11, 2, 2, C.knight); // hand on lance
  ...
  function render() {
    ...
    // Crisp pixel sprites like the original raster game
