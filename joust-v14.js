/**
 * JOUST — Williams Electronics 1982 (Build V14)
 * Rounder Pac/DigDug-hybrid sprites, solid Web Audio (await resume + musicLock).
...
(() => {
  ...
  const C = {
    ...
    lavaCore: "#ffe060",
    yellow: "#ffff40",
    player: "#f0d020", // yellow ostrich (player 1)
    playerDk: "#c09010",
    playerWing: "#e8c018",
    white: "#f0f0f0",
    bounder: "#e03830", // red Bounder (original)
    bounderDk: "#901810",
    hunter: "#c8c8d0", // silver/gray Hunter
    hunterDk: "#686878",
    shadow: "#4040d0", // blue Shadow Lord
    shadowDk: "#202078",
    egg: "#40c040",
    ptero: "#c09070",
    pteroWing: "#a07050",
  ...
  // ── Platforms — classic single-screen Joust arena ───────────────────────
  ...
  function layoutForWave(n) {
    ...
    const restored = n % 5 === 0; // egg waves restore the full rock
    if (!restored && n >= 3) {
      list = list.filter((p) => !(p.kind === "ground" && p.x === 300));
    }
    if (!restored && n >= 6) {
      list = list.filter((p) => !(p.x === 350 && Math.abs(p.y - (VH - 400)) < 2));
    }
    if (!restored && n >= 7) {
      list = list.filter(
        (p) =>
          !(p.x === 100 && Math.abs(p.y - (VH - 300)) < 2) &&
          !(p.x === 690 && Math.abs(p.y - (VH - 300)) < 2)
      );
    }
    PLATFORMS = list;
  }
  ...
  function goTitle() {
    ...
    ptero = null;
    hands = [];
    showOV("JOUST", "INSERT COIN", "BUILD V14 — INSERT COIN — PRESS SPACE / FLAP");
    hud();
  }
  ...
  function attractThink(dt) {
    ...
    const tooLow = player.y > VH - 170 || (t && player.y > t.y + 12);
  ...
  function defeatEnemy(e) {
    ...
    // V14: egg sails/bounces like cabinet — inherit velocity + pop, not a dead drop
    const face = e.face || (e.vx >= 0 ? 1 : -1);
    eggs.push({
      x: e.x,
      y: e.y,
      vx: e.vx * 0.85 + face * rnd(70, 150),
      vy: rnd(-240, -150),
      life: 12000,
      onGround: false,
      bounces: 0,
  ...
  function updatePteros(dt) {
    ...
    for (const pt of pteros) {
      ...
      if (player && player.alive) {
        const targetY = clamp(player.y + Math.sin(pt.frame / 280) * 12, 70, VH - 90);
  ...
  function update(dt) {
    ...
    // Eggs — sail through air, bounce on ledges, then slow-roll (cabinet / 7800 manual)
    for (const egg of eggs) {
      if (egg.dead) continue;
      const dtS = dt / 1000;

      if (!egg.onGround) {
        egg.vy += GRAV * 0.95 * dtS;
        egg.x = wrapX(egg.x + egg.vx * dtS);
        egg.y += egg.vy * dtS;
        // light air drag (still sails)
        egg.vx *= Math.pow(0.55, dtS);
        ...
        for (const p of PLATFORMS) {
          if (
            ...
            // bouncy ledges — several hops before settling
            const bounce = Math.abs(egg.vy) * (egg.bounces < 3 ? 0.62 : 0.38);
            egg.vy = -bounce;
            egg.vx *= 0.9;
            // keep a little roll energy on each hop
            if (Math.abs(egg.vx) < 40) egg.vx += (egg.vx >= 0 ? 1 : -1) * 25;
            if (bounce < 55 && Math.abs(egg.vx) < 55) {
              egg.vy = 0;
              egg.onGround = true;
            }
            sfx("thunk");
        ...
        // rest / slow roll on ledge; fall off ends like the original
        egg.vy = 0;
        egg.x = wrapX(egg.x + egg.vx * dtS);
        egg.vx *= Math.pow(0.12, dtS);
        if (Math.abs(egg.vx) < 6) egg.vx = 0;
  ...
  function drawRider(r, color, dark, wingCol) {
    ...
    // outer silhouette soft edge (puffier / less blocky)
    ctx.fillStyle = "rgba(0,0,0,0.16)";
    ctx.beginPath();
    ctx.ellipse(0.4, 1.0, 14.0, 9.4, -0.06, 0, Math.PI * 2);
    ctx.fill();

    // belly (rounder undercarriage)
    radEll(1.2, 3.8, 12.2, 7.4, belly, lo);
    // main body mass — fuller ellipse
    const bg = ctx.createRadialGradient(-2, -2.2, 1.2, 0.2, 0.8, 15);
    bg.addColorStop(0, hi);
    bg.addColorStop(0.42, color);
    bg.addColorStop(1, deep);
    ctx.fillStyle = bg;
    ctx.beginPath();
    ctx.ellipse(0.1, 0.6, 12.6, 8.2, -0.08, 0, Math.PI * 2);
    ctx.fill();
    // chest puff
    radEll(5.2, 1.0, 7.0, 6.2, hi, mid);
    // back ridge soft shadow
    radEll(-5.8, 0.4, 7.0, 5.6, mid, deep);
    ...
    // skull
    radEll(16.6, -17.6, 6.9, 5.6, hi, lo);
    radEll(15.9, -19.2, 4.5, 3.0, hi, mid);
    // cheek
    radEll(17.6, -16.3, 3.5, 2.7, mid, lo);
    // beak upper mandible
    ...
    if (flap) {
      ...
      ctx.beginPath();
      ctx.moveTo(-2.2, 1.2);
      ctx.bezierCurveTo(-15, -3, -16, -15, -5, -23);
      ctx.bezierCurveTo(1, -25.5, 11, -21, 10, -10);
      ctx.bezierCurveTo(9, -3.5, 4.5, 0.5, -2.2, 1.2);
      ctx.closePath();
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(-8.2, -0.2);
      ctx.bezierCurveTo(-11, 1.5, -10.5, 7.5, -2.2, 8.6);
      ctx.bezierCurveTo(4.2, 7.5, 6.5, 3.2, 4.2, 0.3);
      ctx.bezierCurveTo(1.2, 2.2, -3.2, 2.2, -8.2, -0.2);
      ctx.closePath();
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(-2.8, 0.8);
      ctx.bezierCurveTo(-17, 2.5, -16.5, 14, -4.2, 14.5);
      ctx.bezierCurveTo(4.5, 13, 8.5, 5.2, 5.2, 0.8);
      ctx.bezierCurveTo(2.2, 3.2, 0.2, 2.2, -2.8, 0.8);
      ctx.closePath();
      ctx.fill();
      ctx.strokeStyle = "rgba(0,0,0,0.18)";
      ctx.lineWidth = 0.7;
    ...
    // dome — fuller circle
    const hg = ctx.createRadialGradient(-1.2, -22.5, 0.6, 0.5, -20.8, 7.5);
    hg.addColorStop(0, "#ffffff");
    hg.addColorStop(0.38, armor);
    hg.addColorStop(1, armorDk);
    ctx.fillStyle = hg;
    ctx.beginPath();
    ctx.ellipse(0.5, -21.2, 5.1, 5.5, 0, 0, Math.PI * 2);
    ctx.fill();
    // helmet crest ridge
    ctx.strokeStyle = "rgba(255,255,255,0.35)";
    ctx.lineWidth = 1.2;
  ...
  function drawEgg(egg) {
    // Soft Williams green egg — rounder Pac-hybrid silhouette
    ctx.fillStyle = "rgba(0,0,0,0.22)";
    ctx.beginPath();
    ctx.ellipse(egg.x + 0.4, egg.y + 1.2, 5.6, 2.0, 0, 0, Math.PI * 2);
    ctx.fill();
    const g = ctx.createRadialGradient(egg.x - 1.4, egg.y - 2.4, 0.8, egg.x, egg.y, 7.5);
    g.addColorStop(0, "#a8ffa0");
    g.addColorStop(0.45, C.egg);
    g.addColorStop(1, "#184818");
    ctx.fillStyle = g;
    ctx.beginPath();
    ctx.ellipse(egg.x, egg.y, 5.6, 6.8, 0, 0, Math.PI * 2);
    ctx.fill();
    // soft rim
    ctx.strokeStyle = "rgba(255,255,255,0.22)";
    ctx.lineWidth = 1.1;
    ctx.beginPath();
    ctx.ellipse(egg.x, egg.y, 5.4, 6.5, 0, 0, Math.PI * 2);
    ctx.stroke();
    ...
    ctx.lineCap = "round";
    ctx.beginPath();
    ctx.ellipse(egg.x - 1.2, egg.y - 2.2, 2.4, 2.8, -0.45, 0.15, Math.PI * 0.95);
    ctx.stroke();
  }
  ...
  function drawOnePtero(pt) {
    ...
    ctx.fillStyle = bg;
    ctx.beginPath();
    ctx.ellipse(0, 0.5, 13.5, 5.8, -0.08, 0, Math.PI * 2);
    ctx.fill();
    // belly
    ...
    ctx.beginPath();
    ctx.moveTo(20, -3.5);
    ctx.bezierCurveTo(26, -5, 32, -3.5, 36.5, -1.5);
    ctx.quadraticCurveTo(32, -0.5, 22, -0.2);
    ctx.closePath();
    ctx.fill();
  ...
  function drawPteroWing(side, wingUp, wingAmt, membrane, membraneLo, membraneHi, far) {
    ...
    ctx.moveTo(0, 0);
    if (wingUp) {
      ctx.bezierCurveTo(-9, -3.5, -18, -11, -7, -21.5);
      ctx.bezierCurveTo(1.5, -20, 11.5, -13, 9, -4);
      ctx.bezierCurveTo(6.5, -1.5, 3, 0.2, 0, 0);
    } else {
      ctx.bezierCurveTo(-11, 2.5, -19.5, 11, -8.5, 19);
      ctx.bezierCurveTo(0.5, 17.5, 11, 11, 8.5, 3.2);
      ctx.bezierCurveTo(5.5, 1.2, 2.2, 0.2, 0, 0);
    }
    ctx.closePath();
    ctx.fill();
  ...
  function render() {
    ...
    for (const e of enemies) {
      if (!e.alive) continue;
      if (e.type === "hunter") drawRider(e, C.hunter, C.hunterDk, "#e8e8f0");
      else if (e.type === "shadow") drawRider(e, C.shadow, C.shadowDk, "#7878e8");
      else drawRider(e, C.bounder, C.bounderDk, "#ff6860");
    }
    if (player && player.alive) {
