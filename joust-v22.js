/**
 * JOUST — Williams Electronics 1982 (Build V22)
 * V22: arcade board/sprites/gameplay/audio polish. Cliff X = V19 (byte×2×3). Egg physics = V15.
 * Thicker rocky cliffs + lava-troll island, rounder sprites, mild arcade retune, stronger synth.
 */
(() => {
  "use strict";
  ...
  // Exact 3Ã— Williams arcade playfield (292Ã—240 â†’ 876Ã—720)
  const VW = 876; // 292*3
  const VH = 720; // 240*3
  const DPR = Math.min(window.devicePixelRatio || 1, 3);
  // ~3Ã— backing store â†’ crisp anti-aliased vectors on retina / large screens
  ...
  // â”€â”€ Colors closer to Williams 16-color Joust feel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const C = {
    black: "#000000",
    sky: "#000000",
    skyGlow: "#1a0c18",
    ...
    lavaCore: "#ffe060",
    yellow: "#ffff40",
    // Bird BODY colors (Williams attract) â€” rider armor is separate in drawRider
  ...
  // â”€â”€ Audio â€” original Williams cabinet DAC samples (not 2600 beeps)
  ...
  function sfx(name) {
    unlockAudio();
    if (muted || !AC) return;
    // V22: Web Audio synth is primary (always audible). Optional cabinet wav layers only.
    if (musicLocked() && (name === "wave" || name === "extra" || name === "ptero")) return;
    if (name === "flap") {
      playSample("flap", 0.55, 1);
      noise(0.085, 0.085, 0, 1400, 180);
      tone(190, 0.09, "dac", 0.062, 0, 55);
      tone(95, 0.06, "triangle", 0.028, 0.01, 48);
    } else if (name === "trot" || name === "skid") {
      const now = AC.currentTime;
      const gap = name === "skid" ? 0.08 : 0.11;
      if (now - lastTrot < gap) return;
      lastTrot = now;
      playSample("thunk", name === "skid" ? 0.22 : 0.2, name === "skid" ? 1.2 : 1.55);
      noise(name === "skid" ? 0.055 : 0.04, name === "skid" ? 0.055 : 0.042, 0, name === "skid" ? 650 : 950, name === "skid" ? 160 : 380);
      if (name === "skid") tone(110, 0.05, "sawtooth", 0.022, 0, 55);
    } else if (name === "kill") {
      playSample("thunk", 0.7, 1);
      noise(0.16, 0.095, 0, 780, 140);
      tone(340, 0.18, "dac", 0.068, 0, 55);
      tone(180, 0.12, "square", 0.02, 0.04, 70);
    } else if (name === "die") {
      playSample("thunk", 0.55, 0.85);
      playSample("lava", 0.4, 1.05);
      noise(0.55, 0.11, 0, 520, 60);
      tone(260, 0.55, "dac", 0.072, 0, 32);
      tone(90, 0.4, "sawtooth", 0.035, 0.08, 28);
    } else if (name === "egg") {
      playSample("egg", 0.5, 1);
      tone(560, 0.09, "dac", 0.06, 0, 860);
      tone(740, 0.07, "triangle", 0.035, 0.04, 980);
    } else if (name === "hatch") {
      playSample("egg", 0.45, 0.72);
      noise(0.14, 0.07, 0, 1900, 420);
      tone(300, 0.1, "dac", 0.04, 0, 160);
    } else if (name === "wave") {
      playSample("freeman", 0.55, 1);
      [150, 200, 270, 360, 450, 560].forEach((f, i) => tone(f, 0.18, "dac", 0.05, i * 0.095, f * 1.4));
    } else if (name === "ptero") {
      playSample("ptero", 0.55, 1);
      tone(200, 0.6, "sawtooth", 0.055, 0, 55);
      noise(0.55, 0.06, 0, 520, 120);
      tone(90, 0.35, "dac", 0.03, 0.1, 40);
    } else if (name === "lava") {
      playSample("lava", 0.5, 1);
      noise(0.48, 0.1, 0, 200, 45);
      tone(65, 0.42, "sawtooth", 0.055, 0, 28);
      tone(40, 0.3, "triangle", 0.03, 0.05, 22);
    } else if (name === "bounce") {
      playSample("thunk", 0.5, 1.12);
      noise(0.07, 0.055, 0, 380, 100);
      tone(220, 0.08, "dac", 0.04, 0, 90);
    } else if (name === "extra") {
      playSample("freeman", 0.6, 1.05);
      [300, 380, 480, 600, 740, 900].forEach((f, i) => tone(f, 0.11, "dac", 0.05, i * 0.07, f * 1.22));
    } else if (name === "start") {
      lockMusic(2.4);
      playSample("start", 0.55, 1);
      [95, 140, 185, 245, 320, 410].forEach((f, i) => tone(f, 0.22, "dac", 0.055, i * 0.12, f * 1.5));
    } else if (name === "buzz") {
      // enemy flap / buzzard wing
      noise(0.06, 0.05, 0, 900, 250);
      tone(140, 0.07, "sawtooth", 0.04, 0, 70);
    }
  }
  ...
  // â”€â”€ Platforms â€” exact 3Ã— arcade cliff IDs from playfield reference â”€â”€â”€â”€â”€â”€â”€
  // Williams dest X=byte-col*2px, Y=pixel. Arcade y: CLIF1=69, CLIF2=81, CLIF3U=129, CLIF3L/R=138, CLIF4=163, CLIF5=211 â†’ Ã—3
  const BASE_PLATFORMS = [
    // TOP-LEFT + TOP-RIGHT small ledges
    { x: 6, y: 207, w: 102, h: 14, kind: "float", cliff: "CLIF1L" },
    { x: 756, y: 207, w: 144, h: 14, kind: "float", cliff: "CLIF1R" },
    // WIDE center float (upper)
    { x: 258, y: 243, w: 264, h: 16, kind: "float", cliff: "CLIF2" },
    // Medium CENTER float + LEFT/RIGHT mid floats
    { x: 606, y: 387, w: 174, h: 16, kind: "float", cliff: "CLIF3U" },
    { x: 6, y: 414, w: 192, h: 16, kind: "float", cliff: "CLIF3L" },
    { x: 762, y: 414, w: 144, h: 16, kind: "float", cliff: "CLIF3R" },
    // Lower medium center float
    { x: 318, y: 489, w: 192, h: 16, kind: "float", cliff: "CLIF4" },
    // Thin bridges flush with CLIF5 top (collapse nâ‰¥3) + big ground island
    { x: 0, y: 628, w: 162, h: 16, kind: "bridge", cliff: "BRIDGE_L" },
    { x: 162, y: 633, w: 558, h: 54, kind: "ground", cliff: "CLIF5" },
    { x: 720, y: 628, w: 156, h: 16, kind: "bridge", cliff: "BRIDGE_R" },
  ];
  ...
  // â”€â”€ Physics constants (arcade-paced) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const GRAV = 505; // V22: slightly heavier toward arcade drop
  const FLAP_VY = -128; // V22: stronger single flap (arcade punch)
  const MAX_FALL = 310;
  const MAX_RISE = -290;
  const ACCEL_X = 420;
  const MAX_VX = 198;
  const GROUND_FRIC = 0.76; // V22: more platform skid
  const AIR_DRAG = 0.994;
  const RIDER_R = 10; // KEEP hitbox; visual scale separate in drawRider
  ...
  function hud() {
    ...
    if ($lives) {
      let s = "";
      for (let i = 0; i < Math.max(0, lives); i++) s += "â–²";
      $lives.textContent = s || "â€”";
    }
  }
  ...
  // â”€â”€ Collision helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function pickSpawnPad() {
    // Arcade-style res point: bottom ground island if present, else best remaining ledge
    const list = PLATFORMS.length ? PLATFORMS : BASE_PLATFORMS;
    const clif5 = list.find((p) => p.cliff === "CLIF5");
    if (clif5) return clif5;
    const prefer = ["CLIF4", "CLIF3U", "CLIF2", "CLIF3L", "CLIF3R", "CLIF1L", "CLIF1R"];
    for (const id of prefer) {
      const p = list.find((x) => x.cliff === id);
      if (p) return p;
    }
    return list[0] || BASE_PLATFORMS[0];
  }

  function spawnPlayer(safe) {
    const pad = pickSpawnPad();
    player = {
      x: pad.x + pad.w * 0.5,
      y: pad.y - RIDER_R - 1,
      vx: 0,
  ...
  // Arcade wave schedule (Sean Riddle / Williams): platform digits
  // 1=CLIF1L 2=CLIF1R 3=CLIF2 4=CLIF4. Bridges only on waves 1–2.
  const WAVE_TABLE = [
    null,
    {b:3,h:0,s:0,p:0,miss:"",type:"Normal"},
    {b:4,h:0,s:0,p:0,miss:"",type:"Survival"},
    {b:6,h:0,s:0,p:0,miss:"",type:"Normal"},
    {b:3,h:3,s:0,p:0,miss:"",type:"Gladiator"},
    {b:12,h:0,s:0,p:0,miss:"",type:"Egg"},
    {b:3,h:3,s:0,p:0,miss:"3",type:"Normal"},
    {b:2,h:4,s:0,p:0,miss:"123",type:"Survival"},
    {b:0,h:6,s:0,p:1,miss:"123",type:"Pterodactyl"},
    {b:0,h:6,s:0,p:0,miss:"1234",type:"Gladiator"},
    {b:12,h:0,s:0,p:0,miss:"",type:"Egg"},
    {b:3,h:5,s:0,p:0,miss:"",type:"Normal"},
    {b:2,h:6,s:0,p:0,miss:"4",type:"Survival"},
    {b:0,h:7,s:0,p:1,miss:"1234",type:"Pterodactyl"},
    {b:0,h:8,s:0,p:0,miss:"1234",type:"Gladiator"},
    {b:0,h:12,s:0,p:0,miss:"",type:"Egg"},
    {b:0,h:5,s:1,p:0,miss:"",type:"Normal"},
    {b:0,h:5,s:1,p:0,miss:"",type:"Survival"},
    {b:0,h:5,s:1,p:2,miss:"",type:"Pterodactyl"},
    {b:0,h:4,s:2,p:0,miss:"",type:"Gladiator"},
    {b:0,h:12,s:0,p:0,miss:"",type:"Egg"},
    {b:0,h:3,s:3,p:0,miss:"12",type:"Normal"},
    {b:0,h:2,s:4,p:0,miss:"12",type:"Survival"},
    {b:0,h:2,s:4,p:2,miss:"124",type:"Pterodactyl"},
    {b:0,h:2,s:4,p:0,miss:"124",type:"Gladiator"},
    {b:0,h:12,s:0,p:0,miss:"",type:"Egg"},
    {b:0,h:3,s:5,p:0,miss:"1234",type:"Normal"},
    {b:0,h:3,s:5,p:0,miss:"1234",type:"Survival"},
    {b:0,h:2,s:4,p:2,miss:"1234",type:"Pterodactyl"},
    {b:0,h:3,s:5,p:0,miss:"1234",type:"Gladiator"},
    {b:0,h:12,s:0,p:0,miss:"",type:"Egg"},
    {b:0,h:4,s:4,p:0,miss:"",type:"Normal"},
    {b:0,h:2,s:6,p:0,miss:"",type:"Survival"},
    {b:0,h:2,s:4,p:2,miss:"3",type:"Pterodactyl"},
    {b:0,h:2,s:6,p:0,miss:"3",type:"Gladiator"},
    {b:0,h:12,s:0,p:0,miss:"",type:"Egg"},
    {b:0,h:2,s:6,p:0,miss:"4",type:"Normal"},
    {b:0,h:0,s:8,p:0,miss:"4",type:"Survival"},
    {b:0,h:0,s:6,p:2,miss:"34",type:"Pterodactyl"},
    {b:0,h:0,s:8,p:0,miss:"34",type:"Gladiator"},
    {b:0,h:12,s:0,p:0,miss:"",type:"Egg"},
    {b:0,h:3,s:7,p:0,miss:"12",type:"Normal"},
    {b:0,h:0,s:10,p:0,miss:"124",type:"Survival"},
    {b:0,h:0,s:7,p:3,miss:"124",type:"Pterodactyl"},
    {b:0,h:0,s:10,p:0,miss:"1234",type:"Gladiator"},
    {b:0,h:12,s:0,p:0,miss:"",type:"Egg"},
    {b:0,h:3,s:7,p:0,miss:"",type:"Normal"},
    {b:0,h:0,s:10,p:0,miss:"",type:"Survival"},
    {b:0,h:0,s:7,p:3,miss:"",type:"Pterodactyl"},
    {b:0,h:0,s:10,p:0,miss:"4",type:"Gladiator"},
    {b:0,h:12,s:0,p:0,miss:"",type:"Egg"},
    {b:0,h:3,s:7,p:0,miss:"12",type:"Normal"},
    {b:0,h:0,s:10,p:0,miss:"124",type:"Survival"},
    {b:0,h:0,s:7,p:3,miss:"124",type:"Pterodactyl"},
    ...
    {b:0,h:0,s:7,p:3,miss:"",type:"Pterodactyl"},
    {b:0,h:0,s:10,p:0,miss:"4",type:"Gladiator"},
    {b:0,h:0,s:12,p:0,miss:"",type:"Egg"},
    {b:0,h:0,s:10,p:0,miss:"12",type:"Normal"},
    {b:0,h:0,s:10,p:0,miss:"124",type:"Survival"},
    {b:0,h:0,s:7,p:3,miss:"124",type:"Pterodactyl"},
    {b:0,h:0,s:10,p:0,miss:"1234",type:"Gladiator"},
    {b:0,h:0,s:12,p:0,miss:"",type:"Egg"},
    {b:0,h:0,s:10,p:0,miss:"",type:"Normal"},
    {b:0,h:0,s:10,p:0,miss:"",type:"Survival"},
    {b:0,h:0,s:7,p:3,miss:"",type:"Pterodactyl"},
    {b:0,h:0,s:10,p:0,miss:"4",type:"Gladiator"},
    {b:0,h:0,s:12,p:0,miss:"",type:"Egg"},
  ...
  function waveSpec(n) {
    const N = WAVE_TABLE.length - 1; // 90
    if (n < 1) n = 1;
    if (n <= N) return WAVE_TABLE[n];
    // After 90, cycle the late pattern (waves 61–90)
    const idx = 61 + ((n - 61) % 30);
    return WAVE_TABLE[idx];
  }

  function enemyTypeForWave(w, i) {
    const spec = waveSpec(w);
    // Build spawn list Bounders → Hunters → Shadow Lords
    const bag = [];
    for (let k = 0; k < spec.b; k++) bag.push("bounder");
    for (let k = 0; k < spec.h; k++) bag.push("hunter");
    for (let k = 0; k < spec.s; k++) bag.push("shadow");
    if (!bag.length) return "shadow";
  ...
  function isSurvivalWave(n) {
    return waveSpec(n).type === "Survival";
  }
  function isEggWave(n) {
    return waveSpec(n).type === "Egg";
  }
  function isPteroWave(n) {
    return waveSpec(n).type === "Pterodactyl" || waveSpec(n).p > 0;
  }
  function isGladiatorWave(n) {
    return waveSpec(n).type === "Gladiator";
  }

  function layoutForWave(n) {
    let list = BASE_PLATFORMS.map((p) => Object.assign({}, p));
    const spec = waveSpec(n);
    // Bridges burn away after wave 2 (arcade); egg waves restore floats but not bridges
    if (n >= 3) {
      list = list.filter((p) => p.cliff !== "BRIDGE_L" && p.cliff !== "BRIDGE_R");
    }
    const miss = spec.miss || "";
    const drop = new Set();
    // Sean Riddle 1–4 map + paired mid ledges so the board clearly changes on-screen
    if (miss.includes("1")) {
      drop.add("CLIF1L");
      drop.add("CLIF3L"); // lower-left companion ledge
    }
    if (miss.includes("2")) {
      drop.add("CLIF1R");
      drop.add("CLIF3R");
    ...
    if (miss.includes("4")) drop.add("CLIF4");
    if (drop.size) {
      list = list.filter((p) => !drop.has(p.cliff));
    }
    // CLIF5 ground island always kept
    PLATFORMS = list;
  }
  ...
  function beginWave(n) {
    ...
    const spec = waveSpec(n);
    layoutForWave(n);
    // Ptero timing: dedicated ptero waves spawn quickly; else hurry-up timer
    if (spec.type === "Pterodactyl" || spec.p > 0) {
      pteroTimer = 900;
      pteroWavePending = Math.max(0, spec.p - 1);
    } else {
    ...
    if (isEggWave(n)) {
      const nEggs = Math.min(12, Math.max(8, spec.b || 12));
      ...
      const count = spec.b + spec.h + spec.s;
    ...
    // Arcade: each new wave starts you at a spawn pad on the current board
    spawnPlayer(true);
    state = "wave";
    waveT = 1800;
    let sub = "PREPARE TO JOUST";
    if (isEggWave(n)) sub = "EGG WAVE — COLLECT ALL EGGS";
    else if (isSurvivalWave(n)) sub = "SURVIVAL WAVE";
    else if (spec.type === "Pterodactyl") sub = "PTERODACTYL WAVE";
    else if (isGladiatorWave(n)) sub = "GLADIATOR WAVE";
    // Hint which ledges vanished this wave
    const miss = spec.miss || "";
    if (n >= 3 && n <= 3) sub = "BRIDGES DESTROYED";
    else if (miss) sub = sub + " — LEDGES DOWN";
  ...
  function goTitle() {
    ...
    ptero = null;
    hands = [];
    showOV("JOUST", "INSERT COIN", "BUILD V22 â€” INSERT COIN â€” PRESS SPACE / FLAP");
    hud();
  }
  ...
  function defeatEnemy(e) {
    ...
    // V15: arcade DEATH3 â€” egg inherits SAME velocities (JOUSTRV4 STEGG), no fake upward pop
    eggs.push({
      x: e.x + 4,
      y: e.y - 8,
      vx: e.vx,
  ...
  // â”€â”€ Physics step for a rider â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function stepRider(r, dt, isPlayer) {
    ...
    if (isPlayer) {
      ...
      if (flapPulse) {
        ...
        if (r._flapHoldAcc >= 90) {
          r._flapHoldAcc = 0;
          r.vy = Math.min(r.vy, 30) + FLAP_VY * 0.55;
          r.flapFrame = 0.75;
          sfx("flap");
        }
      } else {
      ...
      if (r.flapT <= 0) {
        ...
        if (tooLow || fallingFast || nearFloor || chance(0.25)) {
          r.vy = Math.min(r.vy, 40) + FLAP_VY * (r.type === "shadow" ? 1.0 : 0.92);
          r.flapFrame = 1;
          if (chance(0.35)) sfx("buzz");
        }
    ...
    // ceiling â€” soft bounce down, don't pin riders at top
    if (r.y < 50) {
      r.y = 50;
    ...
    if (r.onGround && Math.abs(r.vx) > 10) {
      ...
      if (isPlayer && (prev | 0) !== (r.walkPhase | 0)) {
        if (Math.abs(r.vx) > 120) sfx("skid");
        else sfx("trot");
      }
    } else if (!r.onGround) {
      r.walkPhase = 0;
    ...
    // lava â€” troll grab from wave 4 (original); earlier waves still burn if you fall in
    if (onLava(r.x, r.y + 10)) {
      if (isPlayer) {
        if (wave >= 4) {
          sfx("lava");
          hands.push({ x: r.x, y: LAVA_LINE + 8, life: 650, target: r });
          r.grab = 520; // V22: longer troll struggle
          r.vy = 40;
        } else {
          sfx("lava");
          killPlayer();
  ...
  function updateEnemyAI(e, dt) {
    ...
    if (e.type === "bounder") {
      if (chance(0.4)) e.vx = Math.sign(dx || 1) * (62 + wave * 3.5);
      else e.vx = (chance(0.5) ? 1 : -1) * (56 + wave * 2.5);
    } else if (e.type === "hunter") {
      e.vx = Math.sign(dx || 1) * (78 + wave * 4.5);
      // Prefer being slightly above player â€” flap only if clearly below
      if (e.y > player.y + 24) e.flapT = Math.min(e.flapT, 32);
    } else if (e.type === "shadow") {
      e.vx = Math.sign(dx || 1) * (98 + wave * 5.5);
      // Shadow lords prefer mid-high, not ceiling-camping
      if (e.y > VH * 0.35) e.flapT = Math.min(e.flapT, 40);
  ...
  // â”€â”€ Pterodactyl (original-style intervals + HD art) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function pteroMouthOpen(pt) {
    // Jaw cycles open/closed â€” killable only when open (original lance-to-mouth)
    return Math.sin(pt.mouthPhase) > 0.05; // V22: mouth open window slightly wider
  }

  function pteroMouthPos(pt) {
    // Facing: positive vx = flying right, mouth on right side
    const face = pt.vx >= 0 ? 1 : -1;
    const s = 1.0; // V21 ptero visual // match drawOnePtero scale
    return {
      x: pt.x + face * 30 * s,
      y: pt.y + 0.5 * s,
      face,
  ...
  function updatePteros(dt) {
    ...
    for (const pt of pteros) {
      ...
      if (player && player.alive && invuln <= 0) {
        ...
        const bodyHit = Math.abs(dx) < 28 && Math.abs(dy) < 15;
        ...
        const spearMouth = open && mouthDx < 17 && mouthDy < 12; // V21 mouth hit
  ...
  function update(dt) {
    ...
    // Player grab by lava troll â€” mash flap to escape (original)
    if (player && player.grab) {
      if (state === "attract") attractThink(dt);
      player.grab -= dt;
      if (flapPulse) {
        player.grab -= 165; // V22: flap escapes troll faster
    ...
    // Eggs â€” Williams JOUSTRV4: inherit vel, bounce vy/4 + vx step toward 0, then sit
    for (const egg of eggs) {
      if (egg.dead) continue;
      const dtS = dt / 1000;
  ...
  // â”€â”€ Draw â€” Williams-style arena + animated walk cycle â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function rockShelf(p) {
    ...
    if (bridge) {
      // Obvious wooden bridge over lava pits (removed after wave 2)
      const bh = Math.max(h, 14);
      const g = ctx.createLinearGradient(0, y, 0, y + bh);
      g.addColorStop(0, "#f0b060");
      g.addColorStop(0.3, "#c07030");
    ...
    if (ground) {
      // HOME OF THE LAVA TROLL — heavy craggy island into lava (attract-board feel)
      const bodyH = Math.max(h, 54);
      const bot = Math.min(VH, y + bodyH + 14);
      ctx.beginPath();
      ctx.moveTo(x - 4, y + 6);
      // rounded rocky top silhouette
      const topSteps = Math.max(14, (w / 16) | 0);
      for (let i = 0; i <= topSteps; i++) {
        const t = i / topSteps;
        const px = x + t * w;
        const jag = (i === 0 || i === topSteps) ? 2 : Math.sin(i * 1.7) * 1.6 + Math.sin(i * 0.4) * 0.8;
        ctx.lineTo(px, y + jag);
      }
      ctx.lineTo(x + w + 6, y + bodyH * 0.42);
      // Irregular underside dipping into lava (lava-troll rock mass)
      const botSteps = Math.max(16, (w / 14) | 0);
      for (let i = botSteps; i >= 0; i--) {
        const t = i / botSteps;
        const px = x + t * w;
        const dip =
          Math.sin(i * 1.15 + 0.3) * 12 +
          Math.sin(i * 0.45) * 8 +
          Math.abs(Math.sin(i * 2.2)) * 5;
        const edge = t < 0.08 || t > 0.92 ? 10 : 0;
        ctx.lineTo(px, bot - 6 + dip + edge);
      }
      ctx.lineTo(x - 6, y + bodyH * 0.42);
      ctx.closePath();
      const g = ctx.createLinearGradient(0, y, 0, bot);
      g.addColorStop(0, "#f0b070");
      g.addColorStop(0.12, C.rockLt);
      g.addColorStop(0.35, C.rock);
      g.addColorStop(0.65, C.rockMid);
      g.addColorStop(1, "#2a1008");
      ctx.fillStyle = g;
      ctx.fill();
      // Soft side shading for volume
      const sideL = ctx.createLinearGradient(x, y, x + 40, y);
      sideL.addColorStop(0, "rgba(0,0,0,0.28)");
      sideL.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = sideL;
      ctx.fillRect(x - 2, y + 4, 42, bodyH * 0.7);
      const sideR = ctx.createLinearGradient(x + w, y, x + w - 40, y);
      sideR.addColorStop(0, "rgba(0,0,0,0.32)");
      sideR.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = sideR;
      ctx.fillRect(x + w - 40, y + 4, 44, bodyH * 0.7);

      // Thicker rounded rocky TOP (arcade shelf)
      ctx.fillStyle = "#2a1408";
      roundedBand(x + 2, y - 1, w - 4, 7, "#2a1408");
      const topG = ctx.createLinearGradient(0, y - 1, 0, y + 8);
      topG.addColorStop(0, "#ffe8b0");
      topG.addColorStop(0.35, C.rockRim);
      topG.addColorStop(0.7, C.rockLt);
      topG.addColorStop(1, C.rock);
      ctx.fillStyle = topG;
      ctx.beginPath();
      ctx.moveTo(x + 6, y + 1);
      ctx.quadraticCurveTo(x + w * 0.5, y - 3, x + w - 6, y + 1);
      ctx.lineTo(x + w - 3, y + 6);
      ctx.quadraticCurveTo(x + w * 0.5, y + 9, x + 3, y + 6);
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = "rgba(255,240,200,0.55)";
      ctx.fillRect(x + 10, y + 1.5, w - 20, 1.8);

      // Recessed score wells (attract pad look)
      const wellW = Math.min(78, w * 0.17);
      const wellH = 16;
      const wells = [0.16, 0.42, 0.68];
      for (const wt of wells) {
        const wx = x + w * wt;
        ctx.fillStyle = "rgba(10,4,0,0.62)";
        roundedBand(wx, y + 12, wellW, wellH, "rgba(10,4,0,0.62)");
        ctx.strokeStyle = "rgba(0,0,0,0.45)";
        ctx.lineWidth = 1.2;
        ctx.strokeRect(wx + 0.5, y + 12.5, wellW - 1, wellH - 1);
        ctx.fillStyle = "rgba(220,140,50,0.18)";
        ctx.fillRect(wx + 3, y + 13, wellW - 6, 2.5);
      }

      // HOME OF THE LAVA TROLL label on right face (attract)
      ctx.save();
      ...
      const lx = x + w * 0.62;
      const ly = y + 34;
      ctx.fillText("HOME OF THE", lx, ly);
      ctx.fillText("LAVA TROLL", lx + 4, ly + 11);
      ctx.restore();

      // Rocky cracks / strata
      ctx.strokeStyle = "rgba(30,10,0,0.45)";
      ctx.lineWidth = 1.2;
      for (let i = 28; i < w - 24; i += 32) {
        ctx.beginPath();
        ctx.moveTo(x + i, y + 10);
        ctx.quadraticCurveTo(x + i + 6, y + bodyH * 0.35, x + i + 4, y + bodyH * 0.55);
        ctx.stroke();
      }
      // lava kiss highlights under island
      ctx.fillStyle = "rgba(255,80,0,0.18)";
      for (let i = 0; i < 8; i++) {
        const fx = x + 20 + i * ((w - 40) / 7);
        oval(fx, bot - 4 + Math.sin(i + lavaAnim / 200) * 2, 10, 3, "rgba(255,100,0,0.22)");
      }
      return;
    }

    // Float shelves: THICKER rounded rocky tops (V21 arcade feel)
    const shelf = Math.min(Math.max(h + 4, 16), 22);
    const under = 14 + (Math.sin(x * 0.03) * 3 | 0);
    ctx.beginPath();
    ctx.moveTo(x - 2, y + shelf + under);
    ctx.lineTo(x - 3, y + 5);
    const steps = Math.max(8, (w / 14) | 0);
    for (let i = 0; i <= steps; i++) {
      const t = i / steps;
      const px = x + t * w;
      const jag = (i === 0 || i === steps) ? 1 : Math.sin(i * 2.1 + x * 0.02) * 1.4;
      ctx.lineTo(px, y + jag);
    }
    ...
    for (let i = steps; i >= 0; i--) {
      const t = i / steps;
      const px = x + t * w;
      const jag = 4 + Math.abs(Math.sin(i * 1.6 + x * 0.015)) * 8;
      ctx.lineTo(px, y + 8 + jag);
    }
    ctx.closePath();
    const g = ctx.createLinearGradient(0, y, 0, y + shelf + under);
    g.addColorStop(0, "#ffc888");
    g.addColorStop(0.2, C.rockLt);
    g.addColorStop(0.5, C.rock);
    g.addColorStop(1, C.rockDk);
    ctx.fillStyle = g;
    ctx.fill();

    // Thick rounded TOP cap
    ctx.beginPath();
    ctx.moveTo(x + 3, y + 2);
    ctx.quadraticCurveTo(x + w * 0.5, y - 2.5, x + w - 3, y + 2);
    ctx.lineTo(x + w - 2, y + 7);
    ctx.quadraticCurveTo(x + w * 0.5, y + 10, x + 2, y + 7);
    ctx.closePath();
    const tg = ctx.createLinearGradient(0, y - 2, 0, y + 9);
    tg.addColorStop(0, "#fff0c8");
    tg.addColorStop(0.4, "#f0b070");
    tg.addColorStop(1, C.rock);
    ctx.fillStyle = tg;
    ctx.fill();
    ctx.fillStyle = "rgba(255,255,220,0.45)";
    ctx.fillRect(x + 8, y + 2, Math.max(4, w - 16), 1.6);

    // Rock face texture
    ctx.fillStyle = "rgba(0,0,0,0.3)";
    for (let i = 10; i < w - 8; i += 12) {
      const uy = y + 9 + (i % 5);
      ctx.fillRect(x + i, uy, 3.5, 2.5);
    }
    // Optional gray transporter pads on CLIF2 / CLIF3U
    if (p.cliff === "CLIF2" || p.cliff === "CLIF3U") {
      const padW = Math.min(32, w * 0.24);
      const px1 = x + w * 0.2;
      const px2 = x + w * 0.6;
      roundedBand(px1, y - 3, padW, 5, "rgba(170,170,180,0.6)");
      roundedBand(px2, y - 3, padW, 5, "rgba(170,170,180,0.6)");
      ctx.fillStyle = "rgba(220,220,230,0.45)";
      ctx.fillRect(px1 + 3, y - 1.5, padW - 6, 1.8);
      ctx.fillRect(px2 + 3, y - 1.5, padW - 6, 1.8);
    }
  }

  function drawBackground() {
    // Pure black void â€” no starfield haze (arcade look)
    fillRect(0, 0, VW, VH, C.black);
  }

  function drawLava() {
    const pulse = 0.5 + 0.5 * Math.sin(lavaAnim / 120);
    const pulse2 = 0.5 + 0.5 * Math.sin(lavaAnim / 75 + 1.2);
    // Deep underglow
    fillRect(0, LAVA_LINE - 6, VW, VH - (LAVA_LINE - 6), "#0a0100");
    const glow = ctx.createLinearGradient(0, LAVA_LINE - 42, 0, VH);
    glow.addColorStop(0, "rgba(255,60,0,0)");
    glow.addColorStop(0.25, "rgba(255,90,0," + (0.18 + pulse * 0.12) + ")");
    glow.addColorStop(0.55, "rgba(255,40,0,0.35)");
    glow.addColorStop(1, "rgba(180,10,0,0.55)");
    ctx.fillStyle = glow;
    ctx.fillRect(0, LAVA_LINE - 42, VW, VH - (LAVA_LINE - 42));

    // Layered wave surface
    const lg = ctx.createLinearGradient(0, LAVA_LINE - 2, 0, VH);
    lg.addColorStop(0, "#ffe878");
    lg.addColorStop(0.08, C.lavaCore);
    lg.addColorStop(0.22, C.lavaHot);
    lg.addColorStop(0.5, C.lava);
    lg.addColorStop(0.78, "#801000");
    lg.addColorStop(1, "#280400");
    ctx.fillStyle = lg;
    ctx.beginPath();
    ctx.moveTo(0, LAVA_LINE + 4);
    const segs = 40;
    for (let i = 0; i <= segs; i++) {
      const t = i / segs;
      const wave =
        Math.sin(t * Math.PI * 7 + lavaAnim / 80) * 5.5 * pulse +
        Math.sin(t * Math.PI * 3.2 + lavaAnim / 55) * 3.2 * pulse2;
      ctx.lineTo(t * VW, LAVA_LINE + wave);
    }
    ctx.lineTo(VW, VH);
    ctx.lineTo(0, VH);
    ctx.closePath();
    ...
    // Hot ripple band
    ctx.strokeStyle = "rgba(255,220,80," + (0.35 + pulse * 0.25) + ")";
    ctx.lineWidth = 2;
    ctx.beginPath();
    for (let i = 0; i <= segs; i++) {
      const t = i / segs;
      const wave =
        ...
        Math.sin(t * Math.PI * 3.2 + lavaAnim / 55) * 3.2 * pulse2;
      const px = t * VW;
      const py = LAVA_LINE + wave + 1;
      if (i === 0) ctx.moveTo(px, py);
      else ctx.lineTo(px, py);
    }
    ctx.stroke();

    // Edge flame tips (richer)
    for (let i = 0; i < 28; i++) {
      const fx = 6 + ((lavaAnim / 22 + i * 41) % (VW - 12));
      const fh = 7 + (i % 5) * 2.2 + pulse * 4;
      const fy = LAVA_LINE + 1 - fh * 0.4;
      const fg = ctx.createLinearGradient(fx, fy, fx, LAVA_LINE + 6);
      fg.addColorStop(0, i % 3 === 0 ? "#fff0a0" : C.lavaCore);
      fg.addColorStop(1, C.lava);
      ctx.fillStyle = fg;
      ctx.beginPath();
      ctx.moveTo(fx - 3.5, LAVA_LINE + 5);
      ctx.quadraticCurveTo(fx - 1, fy + 2, fx, fy);
      ctx.quadraticCurveTo(fx + 1, fy + 2, fx + 3.5, LAVA_LINE + 5);
      ctx.closePath();
      ctx.fill();
    }
    // Bubbles / embers
    for (let i = 0; i < 22; i++) {
      const bx = 10 + ((lavaAnim / 28 + i * 47) % (VW - 20));
      const by = LAVA_LINE + 10 + ((i * 7 + lavaAnim / 40) % 22);
      oval(bx, by, 2.2 + (i % 3), 1.6 + pulse, i % 2 ? C.lavaCore : C.lavaHot);
    }
  }
  ...
  // â”€â”€ Ultra-HD vector characters (smooth remake, not pixel art) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  /**
   * High-detail knight + ostrich/buzzard â€” rounder Pac/DigDug-hybrid silhouettes.
  ...
  function drawRider(r, color, dark, wingCol) {
    ...
    // Half of v8 size â€” closer to classic Joust on-screen scale. Sharpness from canvas SCALE.
    const s = 0.78; // V21 visual scale-up
    ...
    // â”€â”€ Body (rounder ostrich/buzzard torso â€” soft capsule silhouette) â”€â”€
    // outer silhouette soft edge (puffier arcade oval)
    ctx.fillStyle = "rgba(0,0,0,0.18)";
    ctx.beginPath();
    ctx.ellipse(0.35, 1.15, 16.0, 11.2, -0.04, 0, Math.PI * 2);
    ctx.fill();

    // belly (chunkier soft undercarriage)
    radEll(1.3, 4.2, 14.0, 8.8, belly, lo);
    ...
    ctx.fillStyle = bg;
    ctx.beginPath();
    ctx.ellipse(0.15, 0.55, 14.4, 9.6, -0.06, 0, Math.PI * 2);
    ctx.fill();
    // chest puff
    radEll(5.8, 1.05, 8.4, 7.2, hi, mid);
    ...
    // â”€â”€ Bird head (rounder skull) â”€â”€
    // skull - small round head (arcade compact)
    radEll(16.8, -17.8, 7.9, 6.7, hi, lo);
    ...
    // â”€â”€ Wing â”€â”€
    if (flap) {
      // fully raised wing â€” rounder membrane arc (Pac/DigDug soft silhouette)
      ...
      ctx.closePath();
      ctx.fill();
      // primary feather tips â€” curved, not angular
      ...
      // folded wing against body â€” rounder lobe
      ...
      // gliding wing out â€” soft crescent
    ...
    // â”€â”€ KNIGHT â€” smaller than mount (classic Joust rider proportion) â”€â”€
    ctx.save();
    ...
    // Torso breastplate â€” rounder capsule human silhouette
    ...
    // â”€â”€ Helmet (rounder dome â€” less blocky) â”€â”€
    // neck gorget
    ...
    // â”€â”€ Lance (polished metal shaft) â”€â”€
  ...
  function drawEgg(egg) {
    // V22: rounder glowing arcade egg (visual only; physics = V15)
    ctx.fillStyle = "rgba(0,0,0,0.28)";
    ctx.beginPath();
    ctx.ellipse(egg.x + 0.3, egg.y + 1.6, 7.0, 2.6, 0, 0, Math.PI * 2);
    ctx.fill();
    const g = ctx.createRadialGradient(egg.x - 1.8, egg.y - 2.8, 0.5, egg.x, egg.y, 9.0);
    g.addColorStop(0, "#d0ffc8");
    g.addColorStop(0.35, "#68e060");
    g.addColorStop(0.75, C.egg);
    g.addColorStop(1, "#0a3010");
    ctx.fillStyle = g;
    ctx.beginPath();
    ctx.ellipse(egg.x, egg.y, 6.9, 8.2, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "rgba(180,255,160,0.4)";
    ctx.lineWidth = 1.6;
    ctx.beginPath();
    ctx.ellipse(egg.x, egg.y, 6.7, 7.9, 0, 0, Math.PI * 2);
    ctx.stroke();
    ctx.strokeStyle = "rgba(255,255,255,0.58)";
    ctx.lineWidth = 1.5;
    ctx.lineCap = "round";
    ctx.beginPath();
    ctx.ellipse(egg.x - 1.5, egg.y - 2.7, 3.0, 3.4, -0.45, 0.12, Math.PI * 0.95);
    ctx.stroke();
    ctx.fillStyle = "rgba(255,255,255,0.65)";
    ctx.beginPath();
    ctx.ellipse(egg.x - 2.0, egg.y - 3.2, 1.25, 0.95, -0.3, 0, Math.PI * 2);
    ctx.fill();
  }


  /**
   * HD pterodactyl â€” same vector style as knights/birds.
  ...
  function drawOnePtero(pt) {
    ...
    // V22: rounder / slightly larger arcade ptero
    const s = 1.0;
    ...
    // â”€â”€ Far wing (draw first, behind body) â”€â”€
    ...
    ctx.fillStyle = bg;
    ctx.beginPath();
    ctx.ellipse(0, 0.5, 15.2, 7.2, -0.06, 0, Math.PI * 2);
    ctx.fill();
    // belly
    ...
    // â”€â”€ Jaws (openable â€” kill target) â”€â”€
    // upper snout
  ...
  function drawPteroWing(side, wingUp, wingAmt, membrane, membraneLo, membraneHi, far) {
    ...
    ctx.fillStyle = wg;
    ctx.beginPath();
    // smoother leathery membrane â€” still readable pterosaur shape
    ctx.moveTo(0, 0);
    if (wingUp) {
  ...
  function render() {
    // Draw in logical space at SCALEÃ— resolution for smooth HD look
  ...
  // V22: resume AudioContext on any first user gesture (silence-bug guard)
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      // Debug/verify: N skips to next wave so board changes are easy to check
      if ((e.key === "n" || e.key === "N") && (state === "play" || state === "wave" || state === "clear")) {
        e.preventDefault();
        enemies = [];
        eggs = [];
        pteros = [];
